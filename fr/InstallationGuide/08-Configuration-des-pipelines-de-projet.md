...
- bash : 
   ...
   echo 'FSDH : Installation de GitPython'
   ...
   echo 'FSDH : Installer les modules PS'
   pwsh -c "Install-Module -Name SqlServer -AllowClobber -Scope CurrentUser -Force"

   echo 'FSDH : Installer le client PostgreSQL'
    ...
    En ligne : 
     ...
     foreach ($file in (git diff --name-only "HEAD~1")){
         ...
         if ($file_path[0] -eq "terraform" -AND $file_path[1] -eq "projects" -AND $file_path[2] -ne "") {
             write-host "FSDH : Changement détecté dans le projet : "$file_path[2]
  ...
  les entrées :
    ...
    En ligne : 
     ...
     foreach ($project in $project_list){
       write-host "FSDH : Traitement du projet $project"
       terraform --version
       pushd $projet
